...
[![OpenShift](https://img.shields.io/badge/OpenShift-4.16--4.22%20|%205.0-ee0000.svg)](https://www.redhat.com/en/technologies/cloud-computing/openshift)
...
```bash
wget https://github.com/sjbylo/aba/archive/refs/tags/v1.2.2.tar.gz
...
```bash
git clone --branch v1.2.2 https://github.com/sjbylo/aba.git
cd aba
./install
aba
```
...
See [Bare-metal Network Planning Example](docs/bare-metal-example.md) for a complete worked example showing a 3-node compact cluster with network topology diagram, DNS records, matching config files (`aba.conf`, `cluster.conf`, `mirror.conf`, `macs.conf`), firewall ports, and a pre-flight checklist.
