...
- **`aba image add/remove/list` documented** — README now includes an "Additional Container Images" section. Help text for `aba` and `aba mirror` lists the `aba image` commands.
- **Operator dependency tools** — `tools/listopdeps.sh` lists recursive OLM dependencies for any operator. `tools/verify-operator-deps.sh` verifies all operator sets have complete dependencies. Both handle the old (`catalog.json`) and new (`bundles/`) FBC catalog formats.
- **Pre-commit: operator set validation** — `build/pre-commit-checks.sh` step 8 validates every operator name in `templates/operator-set-*` against the shipped catalog indexes. Catches typos and removed operators before release.
- **Functional test for operator sets** — `test/func/test-operator-sets.sh` verifies structure, catalog presence, `cincinnati-operator` inclusion, no duplicates, valid names, and per-set dependency completeness.

### Changed

- **ISC ownership UX overhaul** — Generated `imageset-config.yaml` no longer contains the misleading "If you edit this file, ownership transfers to you" invitation. When the user edits the ISC, ABA dynamically inserts a "user-managed" notice with the file path and exact command to regenerate. The header changes from "File generated by ABA" to "File originally generated by ABA". Leading/trailing blank lines and trailing whitespace from the editbox are cleaned up automatically. Deleting the file or using TUI "Regenerate" restores a clean ABA-managed file.
- **Agent/install-config header updated** — Generated `agent-config.yaml` and `install-config.yaml` now say "Your edits will be preserved unless cluster.conf or mirror.conf changes" instead of the old ownership invitation. This accurately describes Make's dependency-based regeneration.
- **Agent-config apiVersion bumped to `v1beta1`** — All `agent-config.yaml` templates now use `apiVersion: v1beta1` (stable since OCP 4.12, no schema change from `v1alpha1`).
- **Branding: "Aba" → "ABA"** — All user-facing text (help, banners, script output, template headers) now consistently uses "ABA" as the project name. The CLI command remains lowercase `aba`.
- **Template headers include GitHub URL** — All generated files now have `# File generated by ABA (github.com/sjbylo/aba)` as the header line.
- **TUI: "Basket" renamed to "Selection"** — All operator-related dialogs, messages, and help text now use "Selection" instead of "Basket" for a more professional tone.
- **`lightspeed-operator` added to `ocp` set** — OpenShift Lightspeed (AI assistant) is now included in the default OpenShift operators set.
- **Operator set names allow hyphens** — The `op_sets` validation regex and `aba ops` listing now correctly handle set names containing hyphens (e.g., `ibmstorage`).
- **Removed `ose-cli` from ABA** — The `ose-cli` container image is not needed (payload CLI + `oc` binary cover it) and `:latest` is not published by Red Hat. Removed from bundles, templates, and examples.

### Fixed

- **TUI: `images.conf` editbox validates image references** — The editbox now rejects invalid entries (junk text, malformed references) before saving, using the same validation as `aba image add`.
...
- **vSphere preflight: missing folder warns instead of aborting** — The folder is created at install time by `vmw-create-folder.sh`, so a missing folder during preflight is now a warning, not a fatal error.
- **TUI: Operator selection missing catalog labels** — `grep -m1` with multiple files returned one match per file, corrupting the display when an operator existed in more than one catalog. Fixed with `head -1` to ensure a single result.
- **TUI: Operator set selection failed silently** — Padded tag names in the checklist broke the read-back trim (single-character strip vs. multi-character padding). Fixed with proper all-whitespace trim.
