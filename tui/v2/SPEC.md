...
1. **──── Mirror ────** — View/Edit ImageSet Config; Select Operators; **Add additional images**; Install Mirror; Sync Images `(mirror2mirror)`
...
## Additional images (`images.conf`)

Architecture (two files, merge/dedupe, ISC comments): [ADR-013](../../devel/adr/013-additional-images-conf.md).

The TUI does **not** write `images.conf` or the ISC. It is a dumb consumer of
`aba image add` (and later `list` / `remove`).

### Dialog

Same pattern as pull-secret paste (`_direct_pull_secret`): `dialog --editbox`,
multi-line free text. One image per line; `#` comments and blanks ignored.
Title/prompt must name the file that will be updated.

Do not infer the target from TUI cwd. The **menu that opened the dialog**
selects the file:

| Opened from | Command | File |
|-------------|----------|------|
| CONNO Mirror section — Add additional images | `aba image add img1 img2 ...` | `images.conf` (next to `aba.conf`) |
| Mirror settings (this mirror) | `aba -d mirror image add ...` | `mirror/images.conf` |

After a successful add, kick background ISC regen like operator-basket
changes (`aba:isconf:generate`). If the ISC is user-managed, show the core
warning; do not overwrite the YAML.

DISCO: View ISC stays read-only; do not offer add-on-disconnected (extras
belong on the connected save/sync side).

Optional later: list/remove UI calling `aba image list` / `aba image remove`.

---

## ISC Background Regeneration

The ImageSet Config (`mirror/data/imageset-config.yaml`) is generated by `aba isconf --dir mirror`.
Its inputs are: `ocp_channel`, `ocp_version`, `ops`, `op_sets`, `ARCH`, `excl_platform`, `ocp_upgrade_to`, and `images.conf` / `mirror/images.conf` (ADR-013).
...
- Additional images added (`aba image add` from the TUI editbox)
