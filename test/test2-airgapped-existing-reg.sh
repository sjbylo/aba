...
[ ! "$target_full" ] && default_target="iso"   # Default is to generate 'iso' only on some tests
...
if ssh $TEST_USER@$int_bastion_hostname "aba --dir $subdir/aba/sno --cmd 'oc get clusterversion'"; then
	...
	test-cmd -h $TEST_USER@$int_bastion_hostname -m "Installing sno cluster" "aba --dir $subdir/aba clux -n sno -t sno --starting-ip 10.0.1.201 --mmem 24 --mcpu 12"  
