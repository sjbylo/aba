...
1. **Tier 1 (aba way):** Checks `~/testing/aba/mirror/.available` and `~/aba/mirror/.available`. If found, runs `aba -d mirror uninstall` -- the proper uninstall path that only affects aba-installed registries.
