...
####	mins=0; until oc get node | grep worker | wc -l | grep ^3$; do sleep 60; let mins=$mins+1; [ $mins -ge 15 ] && return 1; done
	...
	if ! test-cmd -i -h $reg_ssh_user@$int_bastion_hostname -m  "Wait 15 mins for workers to come online" "cd $subdir/aba/$cluster_name; . <(aba shell); mins=0; until oc get node | grep worker | wc -l | grep ^3$; do sleep 60; let mins=\$mins+1; [ \$mins -ge 15 ] && exit 1; done"; then
...
test-cmd -h $reg_ssh_user@$int_bastion_hostname -m  "Waiting ~3 mins for node0 to be reachabe" "i=0; until aba --dir $subdir/aba/$cluster_name ssh --cmd hostname; do let i=\$i+1; [ \$i -gt 18 ] && exit 1; echo -n .; sleep 10; done"
