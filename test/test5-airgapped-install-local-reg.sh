...
test-cmd -m "Sleep 30" read -t 30 xy
...
test-cmd -m "Sleep 2m" read -t 120 xy
test-cmd -h $reg_ssh_user@$int_bastion_hostname -m  "List of Operators" "aba --dir $subdir/aba/sno run --cmd 'oc get packagemanifests'"
...
test-cmd -i -h $TEST_USER@$int_bastion_hostname -m "Delete project 'demo'" "aba --dir $subdir/aba/$cluster_type run --cmd 'oc delete project demo'" 
...
test-cmd -m "Wait 30s for ImageDigestMirrorSet to process" read -t 30 xy
...
test-cmd -m "Sleeping 60s" read -t 60 xy
test-cmd -h $reg_ssh_user@$int_bastion_hostname -m "Output upgrade status" "oc adm upgrade --include-not-recommended" 
...
###test-cmd -m "Wait for cluster to settle" read -t 30 xy
...
### until (( $(oc get node --no-headers | grep -c worker) == 3 )); do sleep 60; done
	...
	####test-cmd -m "Wait for cluster to power down" read -t 30 xy
	...
	test-cmd -m "Wait for cluster to settle" read -t 60 xy
