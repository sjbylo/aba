...
test-cmd -r 20 3 -m "Saving ubi images to local disk on `hostname`" "make -s -C mirror save" 
...
test-cmd -h $reg_ssh_user@$int_bastion -m  "Installing $cluster_type cluster, ready to deploy test app" "make -s -C $subdir/aba $cluster_type"
...
build_and_test_cluster() {
	...
	#####
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Waiting for all cluster operators available?" "make -s -C $subdir/aba/$cluster_name cmd; until make -s -C $subdir/aba/$cluster_name cmd | tail -n +2 |awk '{print \$3}' |tail -n +2 |grep ^False$ |wc -l |grep ^0$; do sleep 10; echo -n .; done"

	test-cmd -h $reg_ssh_user@$int_bastion -m  "Waiting for all cluster operators fully available?" "make -s -C $subdir/aba/$cluster_name cmd; until make -s -C $subdir/aba/$cluster_name cmd | tail -n +2 |awk '{print \$3,\$4,\$5}' |tail -n +2 |grep -v '^True False False$' |wc -l |grep ^0$; do sleep 10; echo -n .; done"

	test-cmd -h $reg_ssh_user@$int_bastion -m  "Show all cluster operators" "make -s -C $subdir/aba/$cluster_name cmd"

	# Restart cluster test 
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Log into cluster" ". <(make -s -C $subdir/aba/$cluster_name login)"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check node status" "make -s -C $subdir/aba/$cluster_name ls"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Shut cluster down gracefully and wait" "yes | make -s -C $subdir/aba/$cluster_name shutdown wait=1"

	####test-cmd -m "Wait for cluster to power down" sleep 30
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Checking for all nodes 'poweredOff'" "until make -s -C $subdir/aba/$cluster_name ls |grep poweredOff |wc -l| grep ^$cnt$; do sleep 10; done"

	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check node status" "make -s -C $subdir/aba/$cluster_name ls"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Start cluster gracefully" "make -s -C $subdir/aba/$cluster_name startup"
	test-cmd -m "Wait for cluster to settle" sleep 30
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Checking for all nodes 'Ready'" "until make -s -C $subdir/aba/$cluster_name cmd cmd='oc get nodes'| grep Ready|grep -v Not|wc -l| grep ^$cnt$; do sleep 10; done"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/$cluster_name cmd cmd='get nodes'"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/$cluster_name cmd cmd='whoami'"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/$cluster_name cmd cmd='version'"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/$cluster_name cmd cmd='get po -A | grep -v -e Running -e Complete'"
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/$cluster_name cmd"
	test-cmd -m "Wait for cluster to settle" sleep 60
...
do
	...
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Deleting '$c' cluster" "make -s -C $subdir/aba/$c delete" 
	test-cmd -h $reg_ssh_user@$int_bastion -m  "Running 'make clean' in $subdir/aba/$c" "make -s -C $subdir/aba/$c clean" 
done
...
test-cmd -h $reg_ssh_user@$int_bastion -m  "Making iso" "make -s -C $subdir/aba/standard iso"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Creating standard cluster" "make -s -C $subdir/aba/standard"

test-cmd -h $reg_ssh_user@$int_bastion -m  "Waiting for all co available?" "make -s -C $subdir/aba/standard cmd; until make -s -C $subdir/aba/standard cmd | tail -n +2 |awk '{print \$3}' |tail -n +2 |grep ^False$ |wc -l |grep ^0$; do sleep 10; echo -n .; done"

# Restart cluster test 
test-cmd -h $reg_ssh_user@$int_bastion -m  "Log into cluster" ". <(make -s -C $subdir/aba/standard login)"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Check node status" "make -s -C $subdir/aba/standard ls"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Shut cluster down gracefully and wait (2/2)" "yes | make -s -C $subdir/aba/standard shutdown wait=1"

#test-cmd -m "Wait for cluster to power down" sleep 60
test-cmd -h $reg_ssh_user@$int_bastion -m  "Checking for all nodes 'poweredOff'" "until make -s -C $subdir/aba/standard ls |grep poweredOff |wc -l| grep ^6$; do sleep 10; echo -n .; done"

test-cmd -h $reg_ssh_user@$int_bastion -m  "Check node status" "make -s -C $subdir/aba/standard ls"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Start cluster gracefully" "make -s -C $subdir/aba/standard startup"
#test-cmd -m "Wait for cluster to settle" sleep 600
test-cmd -m "Wait for cluster to settle" sleep 60
...
test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/standard cmd cmd='get nodes'"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/standard cmd cmd='whoami'"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/standard cmd cmd='version'"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/standard cmd cmd='get po -A | grep -v -e Running -e Complete'"
test-cmd -h $reg_ssh_user@$int_bastion -m  "Check cluster up" "make -s -C $subdir/aba/standard cmd"
test-cmd -m "Wait for cluster to settle" sleep 60
test-cmd -h $reg_ssh_user@$int_bastion -m  "Waiting for all co available?" "make -s -C $subdir/aba/standard cmd; make -s -C $subdir/aba/standard cmd | tail -n +2 |awk '{print \$3}' |tail -n +2 |grep ^False$ |wc -l |grep ^0$"
# Restart cluster test end 

# keep it # test-cmd -h $reg_ssh_user@$int_bastion -m  "Deleting standard cluster" "make -s -C $subdir/aba/standard delete" 
###test-cmd -h $reg_ssh_user@$int_bastion -m  "Stopping standard cluster and wait" "yes|make -s -C $subdir/aba/standard shutdown wait=1" 

test-cmd -h $reg_ssh_user@$int_bastion -m "If cluster up, shutting cluster down and wait" ". <(make -sC $subdir/aba/standard shell) && . <(make -sC $subdir/aba/standard login) && yes|make -sC $subdir/aba/standard shutdown wait=1 || echo cluster shutdown failure"

# keep it # test-cmd -h $reg_ssh_user@$int_bastion -m  "Running 'make clean' in $subdir/aba/stanadard" "make -s -C $subdir/aba/standard clean" 
