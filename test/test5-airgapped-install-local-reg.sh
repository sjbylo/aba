...
####	mins=0; until oc get node | grep worker | wc -l | grep ^3$; do sleep 60; let mins=$mins+1; [ $mins -ge 15 ] && return 1; done
	...
	if ! test-cmd -i -h $reg_ssh_user@$int_bastion_hostname -m  "Wait 20 mins for workers to come online" "cd $subdir/aba/$cluster_name; . <(aba shell); mins=0; until oc get node | grep worker | wc -l | grep ^3$; do sleep 60; let mins=\$mins+1; [ \$mins -ge 20 ] && exit 1; done"; then
