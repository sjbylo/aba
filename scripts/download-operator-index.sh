...
if ! curl -kIL http://registry.redhat.io/redhat/redhat-operator-index:v$ocp_ver_major >/dev/null 2>&1; then
	echo_red "$(date): Error: can't access registry.redhat.io to fetch the operator index.  Aborting."
...
if ! ln $index_file $lock_file >/dev/null 2>&1; then
	echo "$(date): Operator index already downloaded - or in progress - for v$ocp_ver_major"
	exit 0
fi

echo $(date): Downloading the operator index from registry.redhat.io/redhat/redhat-operator-index:v$ocp_ver_major, please wait ...
...
if ! oc-mirror list operators --catalog registry.redhat.io/redhat/redhat-operator-index:v$ocp_ver_major > $index_file; then
	rm -f $lock_file
	echo "$(date): Error: cannot download operator index.  Aborting."
	exit 1
fi
...
echo "$(date): Download operator index for v$ocp_ver_major successfuly" 
