...
# For existing registry, user must provide the '$regcreds_dir/pull-secret-mirror.json' and the '$regcreds_dir/rootCA.pem' files in $regcreds_dir/.
if [ -s "$regcreds_dir/pull-secret-mirror.json" ]; then
	aba_debug "Verifying existing mirror pull secret file: $regcreds_dir/pull-secret-mirror.json"
...
if probe_host "$reg_url/health/instance" "Quay registry health endpoint"; then
	aba_abort \
		...
		"To use this registry, copy its pull secret file and root CA file into '$regcreds_dir/' and try again." \
...
if probe_host "$reg_url/" "registry root endpoint"; then
	aba_abort \
		...
		"If this is your existing registry, copy its pull secret file and root CA file into '$regcreds_dir/' and try again." \
		"The files must be named 'pull-secret-mirror.json' and 'rootCA.pem' respectively." \
		"The pull secret file can also be created and verified using 'aba -d mirror password'" \
		"if you don't recognize: $reg_url, be sure to edit the file: $PWD/mirror.conf!" \
		"See the README.md for further information."
...
# CONT # if [ "$reg_ssh_key" ]; then
	...
	scp -i $reg_ssh_key -F $ssh_conf_file -p $reg_ssh_user@$reg_host:$reg_root/quay-rootCA/rootCA.pem "$regcreds_dir/"
	...
	aba_info "Generating $regcreds_dir/pull-secret-mirror.json file"
	...
	eval cp $reg_root/quay-rootCA/rootCA.pem "$regcreds_dir/"   # eval since $reg_root may container "~"
