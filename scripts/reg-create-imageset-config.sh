...
if [ "${_isc_force:-}" != "no" ] && [ -n "${_isc_force:-}" ] || \
   [ ! -s data/imageset-config.yaml ] || [ ! -f data/.created ] || [ ! data/imageset-config.yaml -nt data/.created ]; then
	...
	if ! grep -q "^# This file .* is user-managed" data/imageset-config.yaml 2>/dev/null; then
		...
		# Clean up whitespace from user editing (leading/trailing blank lines, trailing spaces)
		sed -i -e '/[^[:space:]]/,$!d' -e 's/[[:space:]]*$//' data/imageset-config.yaml
		sed -i -e :a -e '/^[[:space:]]*$/{$d;N;ba' -e '}' data/imageset-config.yaml
		sed -i "s/^# File generated by ABA/# File originally generated by ABA/" data/imageset-config.yaml
		sed -i "/^# File originally generated by ABA/a # This file (${_mir_name}/data/imageset-config.yaml) is user-managed. To regenerate: delete this file, or run aba --force -d ${_mir_name} imagesetconf" data/imageset-config.yaml
	fi
