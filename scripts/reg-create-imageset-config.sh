...
if [ "${_isc_force:-}" != "no" ] && [ -n "${_isc_force:-}" ] || \
   [ ! -s data/imageset-config.yaml ] || [ ! -f data/.created ] || [ ! data/imageset-config.yaml -nt data/.created ]; then
	...
	if ! grep -q "^# This file .* is user-managed" data/imageset-config.yaml 2>/dev/null; then
		...
		# Clean up whitespace, mark as user-managed, insert notice after header
		sed -i -e '/[^[:space:]]/,$!d' -e 's/[[:space:]]*$//' -e :a -e '/^[[:space:]]*$/{$d;N;ba' -e '}' \
			-e "s/^# File generated by ABA/# File originally generated by ABA/" \
			-e "/^# File originally generated by ABA/a # This file (${_mir_name}/data/imageset-config.yaml) is user-managed. To regenerate: delete this file, or run aba --force -d ${_mir_name} imagesetconf" \
