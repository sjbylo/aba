...
if [ "$bundle_dest_file" = "-" ]; then
	echo_cyan "An install bundle will be generated and written to *standard output* (stdout) using the following parameters:" >&2
else
...
if [ "$force" ]; then
	...
	if [ -f $bundle_dest_file ]; then
		echo_red "Deleting existing bundle file: $bundle_dest_file (--force set)" >&2
...
if [ -d mirror/save -a mirror/save/imageset-config-save.yaml -nt mirror/save/.created ]; then
...
if [ "$bundle_dest_file" = "-" ]; then
	...
	echo_cyan "Writing install bundle (tar format) to stdout ..." >&2
...
aba_info "Deleting unwanted CLI install files under cli ..."
