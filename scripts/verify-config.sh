...
if [ "$SNO" -a $num_masters -eq 1 -a $num_workers -ne 0 ]; then
	...
	echo SNO master and worker count is valid 
fi
...
echo "'api_vip' and 'ingress_vip' are defined"
...
if [ ! "$SNO" ]; then
	...
	echo "DNS record for api is valid"
	...
	echo "DNS record for ingress is valid"
else
	...
	echo "DNS record for api (SNO) is valid"
	...
	echo "DNS record for ingress (SNO) is valid"
fi
