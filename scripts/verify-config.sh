...
if [ ! "$SNO" ]; then
	...
	if [ "$api_vip" ] && echo "$api_vip" | grep -q -E '^([0-9]{1,3}\.){3}[0-9]{1,3}$'; then
		aba_info "Value 'api_vip' ($api_vip) is defined."
	else
		if [ ! "$actual_ip_of_api" ]; then
			...
			aba_abort "Value 'api_vip' must be defined for this cluster configuration!" 
		fi
	fi

	# If ingress_vip is defined and an IP address
	if [ "$ingress_vip" ] && echo "$ingress_vip" | grep -q -E '^([0-9]{1,3}\.){3}[0-9]{1,3}$'; then
		aba_info "Value 'ingress_vip' ($ingress_vip) is defined."
	else
		...
		if [ ! "$actual_ip_of_ingress" ]; then
			...
			aba_abort "Value 'ingress_vip' must be defined for this cluster configuration!"
		fi
	fi
else
	[ "$api_vip" -o "$ingress_vip" ] && \
		aba_warning "Values 'api_vip' and 'ingress_vip' are not required for SNO configuration, they will be ignored."
fi
